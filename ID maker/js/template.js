/**
 * template.js — ID Card Template Configuration (Sketch-Based Layout)
 *
 * Layout (top to bottom):
 *   Border → Header strip (logo + event name + hatching)
 *         → Main area (portrait + decorative shapes, wiggly overlaps)
 *         → Footer strip (team name + member name/role + hatching)
 *         → Border
 *
 * Canvas: 900×1200 px (3:4 portrait), ISO/IEC 7810 ID-1 proportions.
 */
...
export function getPortraitRect(template = idCardTemplate) {
...
export function getHeaderRect(template = idCardTemplate) {
  const { widthPx: w, heightPx: h } = template.canvas;
  return {
    x: 0, y: 0,
    width: w,
...
export function getFooterRect(template = idCardTemplate) {
  const { widthPx: w, heightPx: h } = template.canvas;
  return {
    x: 0,
 ...
 */
export function getSoftExclusionZone(template = idCardTemplate, paddingPct = 2) {
  const { widthPx: w, heightPx: h } = template.canvas;
