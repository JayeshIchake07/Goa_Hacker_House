import type { PaletteColors, Shape } from "../types";
import { rectsOverlap, distToRect } from "./template";
...
function getZonePosition(zoneName: string, cw: number, ch: number, rng: () => number): { x: number; y: number } {
...
interface MoodLayerConfig {
  ...
  sizeRange: { minPct: number; maxPct: number };
  opacityRange: [number, number];
  placementZones?: string[];
  rotationRange: [number, number];
  usesGradient?: boolean;
  blurRange?: [number, number];
  // Wiggle / Arc specific overrides
  wiggleAmplitude?: [number, number];
  wiggleFrequency?: [number, number];
  wiggleLineWidth?: [number, number];
}
...
interface MoodPreset {
  ...
  description: string;
  bg: MoodLayerConfig;
  overlay: MoodLayerConfig;
}

export const moodPresets: Record<string, MoodPreset> = {
  corporate: {
    name: 'Corporate',
    icon: '▦',
 ...
 */
function generateBgShape(
  bgConfig: MoodLayerConfig,
  palette: PaletteColors,
  exclusionZone: { x: number; y: number; width: number; height: number } | null,
  cw: number,
  ch: number,
  rng: () => number
): Shape | null {
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    const zone = rngChoice(rng, bgConfig.placementZones || ['anywhere']);
    ...
    const shape: Shape = {
      ...
      blur: bgConfig.blurRange ? rngRange(rng, bgConfig.blurRange[0], bgConfig.blurRange[1]) : 0,
 ...
 */
function generateOverlayShape(
  overlayConfig: MoodLayerConfig,
  palette: PaletteColors,
  cw: number,
  ch: number,
  rng: () => number
  ...
  const ampRange = overlayConfig.wiggleAmplitude || [8, 18];
  const freqRange = overlayConfig.wiggleFrequency || [3, 6];
  const widthRange = overlayConfig.wiggleLineWidth || [1.5, 3];
  ...
  const shape: Shape = {
    ...
    blur: 0,
    // Wiggle-specific
    waveAmplitude: rngRange(rng, ampRange[0], ampRange[1]),
    waveFrequency: rngRange(rng, freqRange[0], freqRange[1]),
    waveLineWidth: rngRange(rng, widthRange[0], widthRange[1]),
    // Arc-specific
...
export function generateShapes(
  moodName: string,
  palette: PaletteColors,
  softExclusionZone: { x: number; y: number; width: number; height: number } | null,
  cw: number,
  ch: number,
  seed: number
): Shape[] {
  ...
  for (let i = 0; i < bgCount; i++) {
    const shape = generateBgShape(mood.bg, palette, softExclusionZone, cw, ch, rng);
